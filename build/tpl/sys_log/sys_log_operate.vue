<template>
  <!-- 用户操作日志查询 -->
  <div class="page-sys_log_operate">
    <div class="app-page-container">
      ...
      <div class="app-page-box">
        <el-table
          ...
          <el-table-column prop="sys_user.username" label="用户名" sortable="custom">
            <template slot-scope="scope">{{ scope.row.sys_user.username }}</template>
          </el-table-column>
          <el-table-column prop="sys_user.name" label="姓名" sortable="custom">
          ...
          <el-table-column prop="operate_time" label="操作时间" width="150px" sortable="custom">
            <template slot-scope="scope">{{ scope.row.operate_time | datetime }}</template>
          </el-table-column>
          <!--<el-table-column-->
            <!--prop="duration"-->
            <!--label="执行时长(秒)"-->
          ...
          <el-table-column prop="trigger_page" label="触发页面" sortable="custom">
            <template slot-scope="scope">{{ scope.row.trigger_page }}</template>
          </el-table-column>
          <el-table-column prop="trigger_event" label="触发事件" sortable="custom">
            <template slot-scope="scope">{{ scope.row.trigger_event }}</template>
          </el-table-column>
          <!--<el-table-column-->
          <!--prop="in_params"-->
          <!--label="业务参数"-->
          <!--sortable="custom">-->
          <!--<template slot-scope="scope">{{ scope.row.in_params }}</template>-->
          <!--</el-table-column>-->
  ...
  export default {
    ...
    methods: {
      ...
      search: function () {
        ...
        let date = this.searchForm.operate_time
        if (date && date.length > 0) {
          this.where.push(`operate_time=${date[0]}~${date[1]}`)
        }
        this.list();
      },
      // 修改分页数量
...
<style>
  .page-sys_log_operate {

  }

  .page-sys_log_operate .el-form-item--mini.el-form-item,
  .page-sys_log_operate .el-form-item--small.el-form-item {
    margin-bottom: 0px;
  }
</style>
